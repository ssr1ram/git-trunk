...
pub fn run(_args: &DeleteArgs, remote_name: &str, store_name: &str, verbose: bool) {
    ...
    debug!("Step 1: Prompting user for confirmation to delete store '{}'", store_name);
    ...
    debug!("Step 4: Checking for {} directory", store_dir_relative_path);
    if trunk_store_dir.exists() {
        debug!("Step 4: Removing {} directory for store '{}'", store_dir_relative_path, store_name);
        ...
        match fs::read_dir(&parent_trunk_dir) {
            Ok(mut entries) => {
                if entries.next().is_none() { // Directory is empty
                    debug!("Step 4b: .trunk directory is empty, removing it.");
                    if let Err(e) = fs::remove_dir(&parent_trunk_dir) {
                        error!("Warning: Failed to remove empty .trunk directory at {}: {}", parent_trunk_dir.display(), e);
                    } else {
                        info!("✓ Step 4b: Empty .trunk directory removed.");
                    }
                } else {
                    debug!("Step 4b: .trunk directory is not empty, retaining it.");
                }
            },
            Err(e) => {
                error!("Warning: Could not read .trunk directory contents at {}: {}", parent_trunk_dir.display(), e);
            }
        }
    }
    ...
    debug!("Step 5: Checking for local ref {}", trunk_ref_name);
    ...
    if local_ref_exists {
        debug!("Step 5: Deleting local ref {}", trunk_ref_name);
    ...
    debug!("Step 6: Checking for remote ref {} on remote '{}'", trunk_ref_name, remote_name);
    ...
    if !remote_ref_check.stdout.is_empty() {
        debug!("Step 6: Deleting remote ref {} on remote '{}'", trunk_ref_name, remote_name);
