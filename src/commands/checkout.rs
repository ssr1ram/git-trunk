...
pub fn run(args: &CheckoutArgs, remote_name: &str, store_name: &str, verbose: bool) {
    ...
    debug!("Step 2: Checking if {} exists locally", trunk_ref_name);
    ...
    if local_ref_exists {
        ...
        debug!("Step 3: Checking if {} exists on remote '{}'", trunk_ref_name, remote_name);
    ...
    debug!("Step 8: Checking if {} directory exists for store '{}'", store_dir_relative_path, store_name);
    if trunk_store_dir.exists() {
        ...
        if should_overwrite {
            debug!("Step 8: Removing existing {} directory for store '{}'", store_dir_relative_path, store_name);
    ...
    debug!("Step 10: Initializing Git repository in {}", store_dir_relative_path);
    ...
    if let Err(e) = run_git_command(Command::new("git").arg("update-ref").arg("-d").arg(temp_store_ref).current_dir(&trunk_store_dir), verbose) {
        error!("Warning: Failed to delete temporary ref {} in {}: {}", temp_store_ref, store_dir_relative_path, e);
    }
